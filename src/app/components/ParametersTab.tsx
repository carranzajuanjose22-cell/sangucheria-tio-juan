...
export function ParametersTab() {
  ...
  const saveServices = (newServices: Service[]) => {
    ...
    localStorage.setItem("pos_services", JSON.stringify(newServices));
  };
    ...
    <div className="space-y-8">
            ...
            {services.map((s) => (
              <tr key={s.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-right">
                  ...
                  <button onClick={() => saveServices(services.filter(x => x.id !== s.id))} className="text-red-600 hover:text-red-900 inline-block">
      ...
      {activeModal === "service" && (
        <ServiceModal
          ...
          onSave={(data) => {
            if (editingId) {
              saveServices(services.map(s => s.id === editingId ? { ...s, ...data } : s));
            } else {
              saveServices([...services, { ...data, id: Date.now().toString() }]);
            }
            closeModal();
          }}
        />
