...
export function Dashboard() {
  ...
  const handleCloseRegister = () => {
    ...
    const closeRecord = {
      ...
      totalExpenses: existingExpenses.reduce((acc: number, exp: any) => acc + exp.amount, 0),
  ...
  // Calculamos el balance mensual vs los gastos fijos
      ...
      <div className={`border-2 rounded-2xl shadow-sm overflow-hidden ${
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
            ...
            <div className="bg-white rounded-xl p-5 border border-gray-200">
              <p className="text-sm text-gray-600 mb-2">Gastos Registrados</p>
            ...
            <div className="bg-white rounded-xl p-5 border border-gray-200">
              ...
              <p className="text-2xl font-bold text-green-600">
                ${registerState?.isOpen ? (registerState.initialCash + totalSalesToday - totalExpensesToday).toFixed(2) : "0.00"}
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm mb-6">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end mb-3 gap-2">
              <div>
                <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                  Cobertura de Gastos Fijos <span className="text-sm font-normal text-gray-500">(Mes Actual)</span>
                </h3>
                <p className="text-sm text-gray-500 mt-1">
                  Balance General: <span className="font-medium text-gray-900">${Math.max(monthlyBalance, 0).toFixed(2)}</span> / Meta: <span className="font-medium text-gray-900">${fixedExpensesTotal.toFixed(2)}</span>
                </p>
              </div>
              {isCovered && (
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-green-100 text-green-800 text-sm font-medium whitespace-nowrap">
            ...
            <div className="w-full bg-gray-100 rounded-full h-4 overflow-hidden shadow-inner">
              <div
                className={`h-full rounded-full transition-all duration-1000 ease-out ${isCovered ? 'bg-green-500' : 'bg-blue-500'}`}
